...
@router.get("/v1/movies/search", response_model=MovieSearchResponse)
async def search_movies(
    q: str = Query(..., min_length=1, max_length=200),
    limit: int = Query(default=10, ge=1, le=50),
) -> MovieSearchResponse:
    if not registry.is_loaded:
        raise HTTPException(status_code=503, detail="Catalog still loading")

    results = await run_in_threadpool(registry.search_movies, q, limit)
    return MovieSearchResponse(query=q, results=[MovieSearchResult(**r) for r in results])
